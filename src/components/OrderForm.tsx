...
const OrderSchema = Yup.object().shape({
  ...
  tel: Yup.number().min(2, "Too Short!").required("Required"),
...
export default function OrderForm() {
  ...
  return (
    <Formik initialValues={initialFormValues} validationSchema={OrderSchema} onSubmit={handleSubmit}>
      {(props) => {
        // ✅ useEffect для автоматичного оновлення ціни за 1 кг при зміні категорії
        useEffect(() => {
          if (props.values.category) {
            props.setFieldValue("pricePerKg", categoryPrices[props.values.category]);
          } else {
            props.setFieldValue("pricePerKg", 0);
          }
        ...
        return (
          <Form className={css.form}>
            ...
            <fieldset className={css.fieldset}>
              ...
              <label htmlFor={`${fieldId}-username`} className={css.label}>Імя</label>
              ...
              <Field type="text" name="tel" id={`${fieldId}-username`} className={css.input} />
            ...
            <fieldset className={css.fieldset}>
              ...
              <p className={css.result}>
                Ціна за 1кг: <strong>{props.values.pricePerKg} zł</strong>
              </p>
              <p className={css.result}>
                Загальна вартість (не враховуючи доставку): <strong>{calculatePrice(props.values.weight, props.values.pricePerKg)} zł</strong>
              </p>
            </fieldset>
            ...
            <button type="submit" className={css.button}>
              {props.isSubmitting ? "Submitting order..." : "Надіслати"}
            </button>
          </Form>
        );
      }}
